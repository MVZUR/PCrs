...
#define IDLE_PIN (1<<PC2)							// <-- pin sygna³u idle
...
#define RPM_PIN (1<<PD2)							// <-- pin pomiaru prêdkoœci obrotowej silnika
#define RPM_SIG_DETECTION (PIND & RPM_PIN) !=0		// makrodef sprawdzaj¹ca, czy jest stan wysoki 1

#define TMP_PORT C									// <-- port pomiaru temperatury
#define TMP_PIN (1<<PC0)							// <-- pin pomiaru temperatury (ADC)

#define FOTO_PORT C									// <-- port czujnika zmierzchowego
#define FOTO_PIN (1<<PC1)							// <-- pin czujnika zmierzchowego (ADC)
